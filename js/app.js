...
// ── CART STATE ──
var cart = JSON.parse(localStorage.getItem('hh_cart') || '[]');
...
function addToCart(product) {
  ...
  if (existing) {
    ...
    cart.push({ id: product.id, name: product.name, price: product.price, image: product.image, shipping_cost: product.shipping_cost || 0, qty: 1 });
  }
  saveCart();
  openCart();
...
function renderCartItems() {
  ...
  cart.forEach(function(item) {
    var imgHtml = item.image ? '<img src="' + item.image + '" alt="' + item.name + '">' : '🛍️';
...
function buildProductCard(p) {
  ...
  var safeShipping = parseFloat(p.shipping_cost) || 0;

  // Stock display
  var stock = (p.stock !== null && p.stock !== undefined) ? parseInt(p.stock) : null;
  var outOfStock = stock !== null && stock <= 0;
  var lowStock = stock !== null && stock > 0 && stock <= 5;

  if (outOfStock) {
    badgeHtml = '<span class="product-card-badge" style="background:#fdecea;color:#C0392B;border-color:#C0392B;">Out of Stock</span>';
  } else if (lowStock && !badgeHtml) {
    badgeHtml = '<span class="product-card-badge" style="background:#fff3cd;color:#856404;border-color:#ffc107;">Only ' + stock + ' left!</span>';
  }

  var addBtn = outOfStock
    ? '<button class="btn btn-dark btn-sm" disabled style="opacity:0.45;cursor:not-allowed;">Out of Stock</button>'
